...
module Mttt where
-- module Mttt (
--   -- * Fichas, bloques y tableros
--     Ficha
--   , Bloque
--   , Tablero
--   -- ** Bloque y Tablero vacíos
--   , bloqueVacio
--   , tableroVacio
--   -- * Jugando
--   -- * Utilidades
...
type Bloque = Array (Int,Int) (Maybe Ficha)

-- | Tipo para un tablero de /meta tres en raya/
type Tablero = Array (Int,Int) Bloque
...
-- | 'Bloque' vacío
bloqueVacio :: Bloque
bloqueVacio = listArray ((1,1),(3,3)) [Nothing | _ <- indicesTablero]

-- | 'Tablero' vacío
tableroVacio :: Tablero
tableroVacio = listArray ((1,1),(3,3)) [bloqueVacio | _ <- indicesTablero]
...
casillasLibresBloque :: Bloque -> [(Int, Int)]
casillasLibresBloque b = map fst $ filter snd [((x, y), isNothing $ b!(x,y)) | (x,y) <- indicesTablero]
...
minimaxBloqueDeep = minimaxMain 10 expandirBloque heurBloque
